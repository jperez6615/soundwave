...
async function getStreamUrl(videoId) {
  try {
    // First try to get the direct YouTube URL
    const res = await fetch(`${API_BASE}/api/stream/${videoId}/url`);
    if (res.ok) {
      const data = await res.json();
      if (data.url) return data.url;
    }
  } catch (e) {
    console.warn('Direct URL failed, using proxy');
  }
  // Fallback to proxy
...
export function useAudioPlayer() {
  ...
  useEffect(() => {
    ...
    // Get stream URL then play
    getStreamUrl(currentTrack.youtubeId).then((url) => {
      console.log('Playing:', url.substring(0, 80) + '...');
      audio.src = url;
      audio.load();
      return audio.play();
    }).then(() => {
