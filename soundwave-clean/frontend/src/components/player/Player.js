...
export default function Player() {
  ...
  useEffect(() => {
    ...
    getLyrics(currentTrack.title, currentTrack.artist)
      .then((data) => { if (data.found) setLyricsData(data); })
      .catch(() => {});
        ...
        <div className="max-w-screen-xl mx-auto flex items-center gap-4">
          ...
          <div className="flex-1 flex flex-col items-center gap-2">
            <div className="flex items-center gap-4 md:gap-6">
              <button onClick={toggleShuffle}
                className={clsx('hidden md:block', isShuffle ? 'text-accent' : 'text-text-muted hover:text-text-primary')}>
                <Shuffle size={16} />
              </button>
              <button onClick={playPrev} className="text-text-secondary hover:text-text-primary">
                <SkipBack size={20} fill="currentColor" />
              </button>
              <button
                onClick={() => setIsPlaying(!isPlaying)}
                className="w-10 h-10 rounded-full bg-text-primary text-surface flex items-center justify-center hover:scale-105 transition-transform"
              >
                {isBuffering
              ...
              <button onClick={playNext} className="text-text-secondary hover:text-text-primary">
                <SkipForward size={20} fill="currentColor" />
              </button>
              <button onClick={cycleRepeat}
                className={clsx('hidden md:block', repeatMode !== 'none' ? 'text-accent' : 'text-text-muted hover:text-text-primary')}>
          ...
          <div className="hidden md:flex items-center gap-3 w-[200px] justify-end">
            ...
            <div className="flex items-center gap-2">
              <button onClick={toggleMute} className="text-text-muted hover:text-text-primary">
